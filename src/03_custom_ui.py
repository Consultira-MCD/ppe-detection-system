...
def main():
    ...
    while True:
        ...
        # Diccionario para el One-Hot Encoding del frame actual
        ...
        if cajas is not None and len(cajas) > 0:
            ...
            for (px1, py1, px2, py2, p_id) in coordenadas_personas:
                ...
                if len(lista_faltas_texto) > 0:
                    hay_infraccion = True
                    # Guardamos el estado exacto (One-Hot) y el ID para mandarlo al CSV si se confirma la multa
                    estado_infraccion = {
